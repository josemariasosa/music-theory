# music-theory
Music theory.

## 1. C major scale

The major scale scale is the most important one. 90% of your practicing time must be in major scale.

### Learning C major scale in the bass

Three fingers strategy

#### pointing finger position

```text
C major scale
                                                                              !
G |-------------------------9-10-12-12-10-9-----------------------------------|
D |-----------------9-10-12-----------------12-10-9---------------------------|
A |---------8-10-12---------------------------------12-10-8-------------------|
E |-8-10-12-------------------------------------------------12-10-8-----------|
    ^pointing
```

#### middle finger

```text
C major scale
                                                                              !
G |--------------------7-9-10-10-9-7------------------------------------------|
D |-------------7-9-10---------------10-9-7-----------------------------------|
A |------7-8-10-----------------------------10-8-7----------------------------|
E |-8-10-------------------------------------------10-8-----------------------|
    ^middle
```

#### pinky finder

```text
C major scale
                                                                              !
G |-------------4-5-7-7-5-4---------------------------------------------------|
D |---------5-7-------------7-5-----------------------------------------------|
A |---5-7-8---------------------8-7-5-----------------------------------------|
E |-8---------------------------------8---------------------------------------|
    ^pinky
```

## 2. A minor scale

The minor scale scale is the most important one. 90% of your practicing time must be in minor scale. And this is real. A minor scale is also knwon as the V grade (the fith).

### Learning A minor scale in the bass

Three fingers strategy. The starting finger position help us lear the real shape of the minor scale.

#### pointing finger position

```text
A minor scale
                                                                              !
G |-----------------4-5-7-7-5-4-----------------------------------------------|
D |-------------5-7-------------7-5-------------------------------------------|
A |-------5-7-8---------------------8-7-5-------------------------------------|
E |-5-7-8---------------------------------8-7-5-------------------------------|
    ^pointing
```

#### middle finger

```text
A minor scale
                                                                              !
G |-----------------4-5-7-7-5-4-----------------------------------------------|
D |-----------3-5-7-------------7-5-3-----------------------------------------|
A |-----3-5-7-------------------------7-5-3-----------------------------------|
E |-5-7-------------------------------------7-5-------------------------------|
    ^middle
```

#### pinky finder

```text
A minor scale
                                                                              !
G |---------------2-4-5-5-4-2-------------------------------------------------|
D |---------2-3-5-------------5-3-2-------------------------------------------|
A |---2-3-5-------------------------5-3-2-------------------------------------|
E |-5-------------------------------------5-----------------------------------|
    ^pinky
```

### Other cool minor scales

```text
F minor scale
                                                                              !
G |-------------------1-3-5-5-3-1---------------------------------------------|
D |-------------1-3-5-------------5-3-1---------------------------------------|
A |-------1-3-4-------------------------4-3-1---------------------------------|
E |-1-3-4-------------------------------------4-3-1---------------------------|
    ^pointing

C major scale
                                                                              !
G |-------------------16-17-19-19-17-16---------------------------------------|
D |-------------17-19-------------------19-17---------------------------------|
A |----17-19-20-------------------------------20-19-17------------------------|
E |-20-------------------------------------------------20---------------------|
    ^pinky
```

## 3. All modes in C

It's good to learn the modes in two ways. First, as invididual scales that have independent sounds and textures. Second, as part of the major/minor scales. In this section, I will prsent the scales of each mode starting in C.

### ionian

We will present the scale of C ionian. However, as part of the C major scale, ionian is the C Major scale, and have the I degree (first). 

```text
C ionian scale
                                                                              !
G |--------------------||-----------------------------------------------------|
D |-------------7-9-10-||-10-9-7----------------------------------------------|
A |------7-8-10--------||--------10-8-7---------------------------------------|
E |-8-10---------------||---------------10-8----------------------------------|

C ionian scale
                                                                              !
G |--------------------7-9-10-10-9-7------------------------------------------|
D |-------------7-9-10---------------10-9-7-----------------------------------|
A |------7-8-10-----------------------------10-8-7----------------------------|
E |-8-10-------------------------------------------10-8-----------------------|
    ^middle
```

### dorian

We will present the scale of C dorian. However, as part of the C major scale, dorian is the D minor scale, and have the ii degree (second).

```text
Dm dorian scale
                                                                              !
G |------------------------||-------------------------------------------------|
D |----------------9-10-12-||-12-10-9-----------------------------------------|
A |----------10-12---------||---------12-10-----------------------------------|
E |-10-12-13---------------||---------------13-12-10--------------------------|

Cm dorian scale
                                                                              !
G |---------------------7-8-10-10-8-7-----------------------------------------|
D |--------------7-8-10---------------10-8-7----------------------------------|
A |---------8-10-----------------------------10-8-----------------------------|
E |-8-10-11---------------------------------------11-10-8---------------------|
    ^pointing
```

### phrygian

We will present the scale of C phrygian. However, as part of the C major scale, phrygian is the E minor scale, and have the iii degree (third).

```text
Em phrygian scale
                                                                              !
G |-------------------------||------------------------------------------------|
D |-------------------12-14-||-14-12------------------------------------------|
A |----------12-14-15-------||-------15-14-12---------------------------------|
E |-12-13-15----------------||----------------15-13-12------------------------|

Cm phrygian scale
                                                                              !
G |------------------------8-10-12-12-10-8------------------------------------|
D |----------------8-10-11-----------------11-10-8----------------------------|
A |--------8-10-11---------------------------------11-10-8--------------------|
E |-8-9-11-------------------------------------------------11-9-8-------------|
    ^pointing
```

### lydian

We will present the scale of C lydian. However, as part of the C major scale, lydian is the F Major scale, and have the IV degree (fourth).

```text
F lydian scale
                                                                              !
G |-------------------------||------------------------------------------------|
D |----------------12-14-15-||-15-14-12---------------------------------------|
A |-------12-14-15----------||----------15-14-12------------------------------|
E |-13-15-------------------||-------------------15-13------------------------|
    ^middle

C lydian scale
                                                                              !
G |--------------------7-9-11-11-9-7------------------------------------------|
D |-------------7-9-10---------------10-9-7-----------------------------------|
A |------7-9-10-----------------------------10-9-7----------------------------|
E |-8-10-------------------------------------------10-8-----------------------|
    ^middle
```

### mixolydian

We will present the scale of C mixolydian. However, as part of the C major scale, mixolydian is the G Major scale, and have the V degree (the fifth).

```text
G mixolydian scale
                                                                              !
G |-----------------||--------------------------------------------------------|
D |-------------3-5-||-5-3----------------------------------------------------|
A |-------3-5-7-----||-----7-5-3----------------------------------------------|
E |-3-5-7-----------||-----------7-5-3----------------------------------------|
    ^pointing

C mixolydian scale
                                                                              !
G |-------------------------9-10-12-12-10-9-----------------------------------|
D |-----------------8-10-12-----------------12-10-8---------------------------|
A |---------8-10-12---------------------------------12-10-8-------------------|
E |-8-10-12-------------------------------------------------12-10-8-----------|
    ^pointing
```

### aeolian

We will present the scale of C aeolian. However, as part of the C major scale, aeolian is the A minor scale, and have the vi degree (sixth).

```text
Am aeolian scale
                                                                              !
G |-----------------||--------------------------------------------------------|
D |-------------5-7-||-7-5----------------------------------------------------|
A |-------5-7-8-----||-----8-7-5----------------------------------------------|
E |-5-7-8-----------||-----------8-7-5----------------------------------------|
    ^pointing

Cm aeolian scale
                                                                              !
G |----------------------7-8-10-10-8-7-------------------------------------|
D |-----------------8-10---------------10-8------------------------------|
A |---------8-10-11-------------------------11-10-8---------------------------|
E |-8-10-11-----------------------------------------11-10-8-----------------|
    ^pointing
```

### locrian

We will present the scale of C locrian. However, as part of the C major scale, locrian is the B minor scale, and have the vii° degree (seventh). Never forget that locrian is special, because is a diminished chord, however the correct way should be "half-diminished" chord.

```text
Bm° minor scale
                                                                              !
G |-------------------||------------------------------------------------------|
D |---------------7-9-||-9-7--------------------------------------------------|
A |--------7-8-10-----||-----10-8-7-------------------------------------------|
E |-7-8-10------------||------------10-8-7------------------------------------|
    ^pointing

Cm° locrian scale
                                                                              !
G |-----------------------8-10-11-11-10-8-------------------------------------|
D |---------------8-10-11-----------------11-10-8-----------------------------|
A |--------8-9-11---------------------------------11-9-8----------------------|
E |-8-9-11-----------------------------------------------11-9-8---------------|
    ^pointing
```

## 4. Harmonizing Major scales.

### Basic

From the bass perspective, going up one fret it is called "half-step". This is because to go from C to D two half-steps are reaquired, aka. a step. If we start from C and use the following pattern of half and whole steps we can build any major scale. In music you can also call them tone and semitone.

```text
Major Scale:

┣━━━━━━╋━━━━━━╋━━━╋━━━━━━╋━━━━━━╋━━━━━━╋━━━╋
1      2      3   4      5      6      7   8
  tone   tone       tone   tone   tone
             semitone                 semitone
```

The sound of 1 and 8 are almost the same. They resonate! This mean that we have move an octave from the original tone. The other seven more tones build what we know as the Major scale.

So, to move from 1 to 2, we have to move a tone. To go from 1 to 3, we have to move 2 tones. To 1 to 3, we have to move a **major third**, which mean 1.5 tones.

#### How to name the tones

##### Diatonic Intervals

| Start Note | End Note | Interval Name  | Semitones | Tones |
|:----------:|:--------:|:--------------:|:---------:|:-----:|
| C          | C        | Perfect Unison | 0         | 0     |
| C          | D        | Major 2nd      | 2         | 1     |
| C          | E        | Major 3rd      | 4         | 2     |
| C          | F        | Perfect 4th    | 5         | 2.5   |
| C          | G        | Perfect 5th    | 7         | 3.5   |
| C          | A        | Major 6th      | 9         | 4.5   |
| C          | B        | Major 7th      | 11        | 5.5   |
| C          | C        | Perfect Octave | 12        | 6     |

##### Chromatic Intervals

| Start Note | End Note |  Interval Name | Semitones | Tones |
|:----------:|:--------:|:--------------:|:---------:|:-----:|
|      C     |     C    | Perfect Unison |     0     |   0   |
|      C     |    C#    |   Aug. Unison  |     1     |  0.5  |
|      C     |    Db    |    Minor 2nd   |     1     |  0.5  |
|      C     |     D    |    Major 2nd   |     2     |   1   |
|      C     |    D#    |    Aug. 2nd    |     3     |  1.5  |
|      C     |    Eb    |    Minor 3rd   |     3     |  1.5  |
|      C     |     E    |    Major 3rd   |     4     |   2   |
|      C     |     F    |   Perfect 4th  |     5     |  2.5  |
|      C     |    F#    |    Aug. 4th    |     6     |   3   |
|      C     |    Gb    |    Dim. 5th    |     6     |   3   |
|      C     |     G    |   Perfect 5th  |     7     |  3.5  |
|      C     |    G#    |    Aug. 5th    |     8     |   4   |
|      C     |    Ab    |    Minor 6th   |     8     |   4   |
|      C     |     A    |    Major 6th   |     9     |  4.5  |
|      C     |    A#    |    Aug. 6th    |     10    |   5   |
|      C     |    Bb    |    Minor 7th   |     10    |   5   |
|      C     |     B    |    Major 7th   |     11    |  5.5  |
|      C     |     C    | Perfect Octave |     12    |   6   |

### Stacking thirds

Stacking thirds is a process we use to build, "to harmonize", all the chords from the major scale.

```text
Major Scale:

┣━━━━━━╋━━━━━━╋━━━╋━━━━━━╋━━━━━━╋━━━━━━╋━━━╋
C      Dm     Em  F      G      Am     Bm° C
I      ii     iii IV     V      vi     vii°
1      2      3   4      5      6      7   8
  tone   tone       tone   tone   tone
             semitone                 semitone
```

#### C or I

Let's talk about C major as a chord. If we start from 1, from C, and we move a **major third**, 2 tones, we land on E. Then, if from 3, we move another third, a minor third, then we reach G, the note in the fifth grade. See what we where doing there? Stacking thirds.

This tell us that the C major, or simply C, chord is generated by using al those notes: C, E, G.

```text
Major Scale:
____1 third_____2 third___
|---major-----|--minor---|
*             *          *
┣━━━━━━╋━━━━━━╋━━━╋━━━━━━╋━━━━━━╋━━━━━━╋━━━╋
C      D      E   F      G      A      B   C
1      2      3   4      5      6      7   8
  tone   tone       tone   tone   tone
             semitone                 semitone
```

There could be many, but these are the most common chords for C using a 4 string bass.

```text
  |    C    |    C    |                                                       !
G |---------|----9----|-------------------------------------------------------|
D |----5----|---------|-------------------------------------------------------|
A |----7----|----10---|-------------------------------------------------------|
E |----8----|----8----|-------------------------------------------------------|
```

The first C is being play: C-E-G, and the second C: C-G-E. Technically the same, but aestetically not.

#### Dm or ii

The D minor, or [Cm, Cmin], chord is generated by using al those notes: D, F, A.

```text
Major Scale:

       |--minor---|---major-----|
       *          *             *
┣━━━━━━╋━━━━━━╋━━━╋━━━━━━╋━━━━━━╋━━━━━━╋━━━╋
C      D      E   F      G      A      B   C
1      2      3   4      5      6      7   8
  tone   tone       tone   tone   tone
             semitone                 semitone
```

The most common chords for D minor using a 4 string bass.

```text
  |    Dm   |    Dm   |                                                       !
G |---------|----10---|-------------------------------------------------------|
D |----6----|---------|-------------------------------------------------------|
A |----7----|----12---|-------------------------------------------------------|
E |----10---|----10---|-------------------------------------------------------|
```

The first Dm is being play: D-F-A, and the second Dm: D-A-F.

#### Em or iii

The E minor, or [Em, Emin], chord is generated by using al those notes: E, G, B. It is also called just third, because it's located in that tone.

```text
Major Scale:

              |--minor---|---major-----|
              *          *             *
┣━━━━━━╋━━━━━━╋━━━╋━━━━━━╋━━━━━━╋━━━━━━╋━━━╋
C      D      E   F      G      A      B   C
1      2      3   4      5      6      7   8
  tone   tone       tone   tone   tone
             semitone                 semitone
```

The most common chords for E minor using.

```text
  |    Em   |    Em   |                                                       !
G |---------|----12---|-------------------------------------------------------|
D |----9----|---------|-------------------------------------------------------|
A |----10---|----14---|-------------------------------------------------------|
E |----12---|----12---|-------------------------------------------------------|
```

The first Dm is being play: E-G-B, and the second Dm: E-B-G.

#### F or IV

The F major, or [F], chord is generated by using al those notes: F, A, C.

```text
Major Scale:

                  |---major-----|--minor---|
                  *             *          *
┣━━━━━━╋━━━━━━╋━━━╋━━━━━━╋━━━━━━╋━━━━━━╋━━━╋
C      D      E   F      G      A      B   C
1      2      3   4      5      6      7   8
  tone   tone       tone   tone   tone
             semitone                 semitone
```

The most common chords for F major using.

```text
  |    F    |    F    |                                                       !
G |---------|----14---|-------------------------------------------------------|
D |----10---|---------|-------------------------------------------------------|
A |----12---|----15---|-------------------------------------------------------|
E |----13---|----13---|-------------------------------------------------------|
```

The first Dm is being play: E-G-B, and the second Dm: E-B-G.

#### G or V

The G major, or [G], chord is generated by using al those notes: G, B, D. This is sometimes called the fifth.

```text
Major Scale:

-minor-|                 |---major-----|-minor-
       *                 *             *
┣━━━━━━╋━━━━━━╋━━━╋━━━━━━╋━━━━━━╋━━━━━━╋━━━╋
C      D      E   F      G      A      B   C
1      2      3   4      5      6      7   8
  tone   tone       tone   tone   tone
             semitone                 semitone
```

The most common chords for G major using.

```text
  |    G    |    G    |                                                       !
G |---------|----4----|-------------------------------------------------------|
D |----0----|---------|-------------------------------------------------------|
A |----2----|----5----|-------------------------------------------------------|
E |----3----|----3----|-------------------------------------------------------|
```

The first G is being play: G-B-D, and the second Dm: G-D-B.

#### Am or vi

The A minor, or [Am, Amin], chord is generated by using al those notes: A, C, E.

```text
Major Scale:

|---major-----|                 |--minor---|
*             *                 *          *
┣━━━━━━╋━━━━━━╋━━━╋━━━━━━╋━━━━━━╋━━━━━━╋━━━╋
C      D      E   F      G      A      B   C
1      2      3   4      5      6      7   8
  tone   tone       tone   tone   tone
             semitone                 semitone
```

The most common chords for A minor using.

```text
  |    Am   |    Am   |                                                       !
G |---------|----3----|-------------------------------------------------------|
D |----2----|---------|-------------------------------------------------------|
A |----3----|----5----|-------------------------------------------------------|
E |----5----|----3----|-------------------------------------------------------|
```

The first Am is being play: A-C-E, and the second Am: A-E-C.

#### Bm° or vii°

The B diminished, or [Bdim, B°], chord, that should have be correctly said: "hald-diminished", is generated by using al those notes: B, D, F.

```text
Major Scale:

-minor-|--minor---|                    |-minor-
       *          *                    *
┣━━━━━━╋━━━━━━╋━━━╋━━━━━━╋━━━━━━╋━━━━━━╋━━━╋
C      D      E   F      G      A      B   C
1      2      3   4      5      6      7   8
  tone   tone       tone   tone   tone
             semitone                 semitone
```

The most common chords for B diminished.

```text
  |    Bm°  |    Bm°  |                                                       !
G |---------|----7----|-------------------------------------------------------|
D |----3----|---------|-------------------------------------------------------|
A |----5----|----8----|-------------------------------------------------------|
E |----7----|----7----|-------------------------------------------------------|
```

The first Bm° is being play: B-D-F, and the second Bm°: B-F-D.

###### Play them all together chanting the names

Walk them all, saying out loud the chord grade, the note, the chord, could be very helpful to remember the names.


---

```text
F minor scale
                                                                              !
G |----------------------------------------------------------------|
D |----------------------------------------------------------------|
A |-----------------------------------------------------------------|
E |-------------------------------------------------------------------------|
    ^pointing
```

```text
C major scale
                                                                              !
G |--------------------------------------------------------------------|
D |-------------------------------------------------------------------|
A |-------------------------------------------------------------------|
E |------------------------------------------------------------------------|
```
```text
        C
e |-----0------|
B |-----1------|
G |-----0------|
D |-----2------|
A |-----3------|
E |------------|
```




---
Albums to study:
	- Aqua Puzzle - Kei Akagi



---


http://www.tonalcentre.org/
https://www.pianoscales.org/minor-harmonizing.html
https://www.pianoscales.org/major-harmonizing.html


http://www.musicopoulos.com/diatonic-intervals.html
http://www.musicopoulos.com/chromatic-intervals.html





```text
Major Scale:

┣━━━━━━╋━━━━━━╋━━━╋━━━━━━╋━━━━━━╋━━━━━━╋━━━╋
1      2      3   4      5      6      7   8
  tone   tone       tone   tone   tone
             semitone                 semitone


Natural Minor:

┣━━━━━━╋━━━╋━━━━━━╋━━━━━━╋━━━╋━━━━━━╋━━━━━━╋
1      2   3      4      5   6      7      8
  tone       tone   tone       tone   tone
      semitone          semitone 


Harmonic Minor:

┣━━━━━━╋━━━╋━━━━━━╋━━━━━━╋━━━╋━━━━━━━━━╋━━━╋
1      2   3      4      5   6         7   8
  tone       tone   tone        tone + semitone
      semitone          semitone      semitone


Melodic Minor:

┣━━━━━━╋━━━╋━━━━━━╋━━━━━━╋━━━━━━╋━━━━━━╋━━━╋
1      2   3      4      5      6      7   8
  tone       tone   tone   tone   tone
      semitone                        semitone
```



°°°°°°°°°°°°°°°°°°°°°°
```text
        C
e |-----0------|
B |-----1------|
G |-----0------|
D |-----2------|
A |-----3------|
E |------------|
```

| Symbol | Technique                                                   |
|--------|-------------------------------------------------------------|
| h      | hammer on                                                   |
| p      | pull off                                                    |
| b      | bend string up                                              |
| r      | release bend                                                |
| /      | slide up                                                    |
| \      | slide down                                                  |
| v      | vibrato (sometimes written as ~)                            |
| t      | right hand tap                                              |
| s      | legato slide                                                |
| S      | shift slide                                                 |
| *      | natural harmonic                                            |
| [n]    | artificial harmonic                                         |
| n(n)   | tapped harmonic                                             |
| tr     | trill                                                       |
| T      | tap                                                         |
| TP     | tremolo picking                                             |
| PM     | palm muting (also written as _)                             |
| \n/    | tremolo arm dip; n = amount to dip                          |
| \n     | tremolo arm down                                            |
| n/     | tremolo arm up                                              |
| /n\    | tremolo arm inverted dip                                    |
| =      | hold bend; also acts as connecting device for hammers/pulls |
| <>     | volume swell (louder/softer)                                |
| x      | on rhythm slash represents muted slash                      |
| o      | on rhythm slash represents single note slash                |
| ·/.    | pick slide                                                  |

